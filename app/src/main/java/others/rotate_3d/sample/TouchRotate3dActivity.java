 ...
 */
public class TouchRotate3dActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_touch_rotate);
    ...
    public void buildClick(View v) {
        ...
        if (!countIsEmpty && !distanceIsEmpty) {
            Toast.makeText(getApplicationContext(), "You cannot specify rotation count and distance together.", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        for (int i = 0; i < menu.size(); i++) {
            ...
            if (item.isCheckable()) {
                if (item.getItemId() == R.id.action_touch) {
                    item.setTitle(isTouchEnabled() ? "Touch Enabled" : "Touch Disabled");
