...
public class TimelineView extends View {
    ...
    private void init(AttributeSet attrs) {
        ...
        if(mMarker == null) {
            mMarker = mContext.getResources().getDrawable(R.drawable.shape_green_maker);
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int widthSize = resolveSizeAndState2x(w, widthMeasureSpec, 0);
        int heightSize = resolveSizeAndState2x(h, heightMeasureSpec, 0);

        setMeasuredDimension(widthSize, heightSize);
        initDrawable();
    }

    // resolveSizeAndState() mehtod is since API 11, which is not available for the 2.x phones
    public static int resolveSizeAndState2x(int size, int measureSpec, int childMeasuredState) {
